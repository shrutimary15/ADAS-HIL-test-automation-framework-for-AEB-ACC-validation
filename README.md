...
adas_hil_framework/
├── config/
│   └── test_config.yaml        # Bench config: thresholds, speed ranges, timing limits
├── can/
│   └── signals.py              # CAN signal definitions (DBC equivalent)
├── framework/
│   ├── hil_interface.py        # HIL bench interface — signal inject/read/timing
│   └── signal_monitor.py       # Real-time signal recording and violation detection
├── tests/
│   ├── test_aeb.py             # 7 AEB test cases
│   └── test_acc.py             # 8 ACC test cases
├── reports/
